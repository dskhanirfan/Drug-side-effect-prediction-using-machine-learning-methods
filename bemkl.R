...
source("bemkl-master/bemkl_supervised_classification_variational_train.R")
source("bemkl-master/bemkl_supervised_classification_variational_test.R")

data <- read.csv(file="../Desktop/thesis/9may.csv",header=T)

ListStideeffects <- c("C0027497","C0018681","C0042963","C0011991","C0012833","C0004093","C0011603","C0015230","C0000737","C0015672")

for(list in ListStideeffects){
  
sideeffect <- as.data.frame(data[,list])
#hist(data[,"C0018681"])
...
if(nrow(classOneValues)<30){
  numOfChunks = 23
  print(numOfChunks)
  
}else if(nrow(classOneValues)>35){
  numOfChunks = 19
}else{
  numOfChunks = 20
}
...
for(r in 1:length(randomIterations)){
  ...
  for(chunks in 1:numOfChunks){
    ...
    randomIndicesClassOne<-sample(1:nrow(classOneValues))
...
print(mean(a))
finalfilename <- paste0("bemkl_J_DIFP",list, "_",Sys.Date(), ".RData")
save(bemkl_J_DIFP, file = finalfilename)
}
